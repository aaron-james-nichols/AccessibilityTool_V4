...
def isochrone(input_lat, input_lon, distances, attributes):
    ...
    for edge in G_projected.edges():
        ...
        if 'geometry' not in edge_attribute_keys:
            ...
            length = ox.distance.euclidean_dist_vec(u_y, u_x, v_y, v_x)
    ...
    length = ox.distance.euclidean_dist_vec(y1, x1, y2, x2)
    ...
    # Everything under here is related to the actual isochrone creation.
    polygons = []
    # for point in points:
    for distance in sorted(distances, reverse = True):
        ...
        iso_graph.graph['crs'] = G_projected.graph['crs']
...
# with open('iso_poly_2.geojson', 'w') as output_file:
#     json.dump(iso_poly_json_all, output_file)
